...
vector<Sentence<Token>> SplitIntoSentences(vector<Token> tokens) {
	...
	auto last = FindSentenceEnd(tokens.begin(), tokens.end());
	vector<Sentence<Token>> split;
	while (true)
	{
		vector<Token> result (make_move_iterator(first), make_move_iterator(last));
		split.push_back(move(result));
		if (last == tokens.end())
			break ;
		first = last;
		last = FindSentenceEnd(last, tokens.end());
	}
	return split;
}
